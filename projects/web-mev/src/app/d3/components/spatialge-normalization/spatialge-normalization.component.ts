...
export class SpatialGENormalizationComponent extends BaseSpatialgeComponent implements OnInit  {
    ...
    //     const normRequest = this.httpClient.get(`${this.API_URL}/resources/${uuid}/contents/?__rowname__=[eq]:${this.geneSearch}`).pipe(
    //         catchError(error => {
    ...
    //     const coordsMetadataRequest = this.httpClient.get(`${this.API_URL}/resources/${this.outputs["coords_metadata"]}/contents/`).pipe(
    //         catchError(error => {
    //             this.isLoading = false;
