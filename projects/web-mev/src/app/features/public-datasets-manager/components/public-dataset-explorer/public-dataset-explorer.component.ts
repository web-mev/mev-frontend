...
export class PublicDatasetExplorerComponent extends GdcRnaseqComponent implements OnChanges, OnInit {
  @Input() query: string = "";
  ...
  fetchDataTarget(datasetTag: string, name_map_key: string): void {
    ...
    let originalTissueQueryURL = `${datasetTag}/?q=*:*&facet=on&facet.field=tissue_or_organ_of_origin&rows=0&facet.gender.sort`;
    this.tissue_types_url = (this.query.length === 0) ? originalTissueQueryURL : `${datasetTag}/?q=${this.query}&facet=on&facet.field=tissue_or_organ_of_origin&facet.gender.sort`;
  ...
  checkboxSelectionTarget(event, name: string, category) {
    if (event['checked']) {
      this.selectedNames.push(name);
      // this.publicDataSetComponent.checkboxStatus['target-rnaseq'][category][name] = true;
      ...
      // this.publicDataSetComponent.checkboxStatus['target-rnaseq'][category][name] = false;
  ...
  onTypeToggleTarget(viewMode: string) {
