...
export class GdcRnaseqComponent {
    ...
    fetchData(datasetTag: string, name_map_key: string): void {
        ...
        } else if (datasetTag === 'gtex-rnaseq') {
    ...
    _createDataset(dataType: string, datasetTag: string) {
        ...
        if (dataType === 'byType') {
            ...
            for (let i in this.selectedNames) {
                ...
                url_suffix = (datasetTag === 'gtex-rnaseq') ?
                    datasetTag + `?q=tissue:"${tissue_name}"&rows=${count}&fl=id,tissue` :
                    datasetTag + `?q=tissue_or_organ_of_origin:"${tissue_name}"&rows=${count}&fl=id,project_id`;
