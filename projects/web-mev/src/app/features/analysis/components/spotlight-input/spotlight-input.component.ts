...
@Component({
    selector: 'spotlight-input',
    templateUrl: './spotlight-input.component.html',
    styleUrls: ['./spotlight-input.component.scss'],
    providers: [{ provide: BaseOperationInput, useExisting: SpotlightInputComponent }],
    changeDetection: ChangeDetectionStrategy.Default
})

export class SpotlightInputComponent extends BaseOperationInput implements OnChanges {
    ...
    createForm() {
        ...
        const configTissueModelField = [
            '',
            [...(input.required ? [Validators.required] : [])]
        ];
        controlsConfig[key] = configTissueModelField;

        key = 'gene_id_choice';
        input = this.operationData.inputs[key];
        this.geneIdTypeField = {
            key: key,
            name: input.name,
            desc: input.description,
            required: input.required,
